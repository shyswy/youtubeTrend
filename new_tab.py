...
def display_video(search):
    if search:
        ...
        if video_id:
            ...
            try:
                # CSV 파일 경로 생성
                base_path = "C:/Users/주현/develop/SWBOOTCAMP/프로젝트/공유"
                video_file_path = os.path.join(base_path, 'youtube_data', f'korea_{category}_videos.csv')
                comments_file_path = os.path.join(base_path, 'youtubeTrend', 'csvCollection', f'KR_{category}_comments_20250421_1017.csv')
                ...
                # video_id와 일치하는 행 찾기
                matching_video = video_df[video_df['video_id'] == video_id]
                ...
                if not os.path.exists(comments_file_path):
                    ...
                    matching_comments = comments_df[comments_df['videoId'] == video_id]
                    print(f"일치하는 댓글 개수: {len(matching_comments)}")  # 디버깅용
                    
                    if not matching_comments.empty:
                        comments_data = matching_comments[['author', 'text', 'likeCount']].to_dict('records')
                    else:
                        comments_data = []
                
                # 값 추출 (에러 처리 추가)
                try:
                    channel_name = matching_video['channel'].iloc[0] if 'channel' in matching_video.columns else "채널 정보 없음"
                    views = f"👁️ {matching_video['views'].iloc[0]}회" if 'views' in matching_video.columns else "👁️ 조회수 정보 없음"
                    likes = f"👍 {matching_video['likes'].iloc[0]}개" if 'likes' in matching_video.columns else "👍 좋아요 정보 없음"
                ...
                print(channel_name, views, likes, description, tags, 11111111111111111111)  # 디버깅용
